...
JobStatusCountersPbtConfig = {
  ...
  # initial_state: { status: 0, retry_count: 0, dead_letter_count: 0 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    start: {
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { status: sut.status, retry_count: sut.retry_count, dead_letter_count: sut.dead_letter_count } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
