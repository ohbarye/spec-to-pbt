...
InventoryStatusProjectionPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { movements: sut.movements.dup, status: sut.status, stock: sut.stock } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
