...
RSpec.describe "sort (stateful scaffold)" do
  ...
  module SortPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in SortPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in SortPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: SortPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: SortPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    SortPbtSupport.validate_config!
  ...
  class SortModel
    ...
    def initial_state
      ...
      SortPbtSupport.initial_state(default_state)
    end
