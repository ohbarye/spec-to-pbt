...
require "pbt"
require_relative "weak_impl" if File.exist?(File.expand_path("weak_impl.rb", __dir__))
...
RSpec.describe "weak (stateful scaffold)" do
  ...
  module WeakPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in WeakPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in WeakPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: WeakPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: WeakPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    WeakPbtSupport.validate_config!
