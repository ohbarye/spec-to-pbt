...
PaymentStatusEventAmountsPbtConfig = {
  ...
  # initial_state: { status: 0, captures: [], remaining_amount: 0, settled_amount: 0 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    open: {
      method: :open,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { captures: sut.captures.dup, status: sut.status, remaining_amount: sut.remaining_amount, settled_amount: sut.settled_amount } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
