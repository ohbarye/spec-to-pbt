...
if File.exist?(File.expand_path("wallet_with_limit_pbt_config.rb", __dir__)) && !defined?(::WalletWithLimitPbtConfig)
  raise "Expected WalletWithLimitPbtConfig to be defined in wallet_with_limit_pbt_config.rb"
end
...
RSpec.describe "wallet_with_limit (stateful scaffold)" do
  # Regeneration-safe customization:
  # - edit wallet_with_limit_pbt_config.rb for SUT wiring and durable API mapping
  # - edit wallet_with_limit_impl.rb for implementation behavior
  # - edit this scaffold only for one-off refinements when needed

  module WalletWithLimitPbtSupport
    module_function
    ...
    def config
      defined?(::WalletWithLimitPbtConfig) ? ::WalletWithLimitPbtConfig : {}
    end
    ...
    def validate_config!
      ...
      raise "Unknown config keys in WalletWithLimitPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in WalletWithLimitPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: WalletWithLimitPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: WalletWithLimitPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    WalletWithLimitPbtSupport.validate_config!

    Pbt.assert(worker: :none, num_runs: 5, seed: 1) do
      Pbt.stateful(
        model: WalletWithLimitModel.new,
        sut: -> { WalletWithLimitPbtSupport.sut_factory(-> { WalletWithLimitImpl.new }).call },
        max_steps: 20
      )
    end
  end

  class WalletWithLimitModel
    def initialize
      @commands = [
        DepositCommand.new,
        WithdrawCommand.new
  ...
  class DepositCommand
    ...
    def arguments
      overridden = WalletWithLimitPbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(WalletWithLimitPbtSupport::ARGUMENTS_OVERRIDE_UNSET)
      Pbt.nil
    end
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      # Alloy predicate body (preview): "#w'.balance=add[#w.balance,1]"
      # Analyzer hints: state_field="balance", size_delta=1, transition_kind=nil, requires_non_empty_state=false, scalar_update_kind=:increment_like, command_confidence=:high, guard_kind=:none, guard_field="balance", guard_constant=nil, rhs_source_kind=:unknown, state_update_shape=:increment
      ...
      return nil if WalletWithLimitPbtSupport.call_verify_override(
        name,
      ...
      raise "Expected incremented value for Wallet#balance" unless after_value == before_value + 1
  ...
  class WithdrawCommand
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      # TODO: translate predicate semantics into postcondition checks
      # Alloy predicate body (preview): "#w.balance>0 implies#w'.balance=sub[#w.balance,1]"
      # Analyzer hints: state_field="balance", size_delta=-1, transition_kind=nil, requires_non_empty_state=true, scalar_update_kind=:decrement_like, command_confidence=:high, guard_kind=:non_empty, guard_field="balance", guard_constant=nil, rhs_source_kind=:unknown, state_update_shape=:decrement
