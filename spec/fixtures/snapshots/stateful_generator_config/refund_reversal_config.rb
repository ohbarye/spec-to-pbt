...
RefundReversalPbtConfig = {
  ...
  # initial_state: { captured: 0, refunded: 0 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    capture: {
      method: :capture,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { captured: sut.captured, refunded: sut.refunded } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
