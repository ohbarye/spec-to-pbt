...
RSpec.describe "hold_capture_release (stateful scaffold)" do
  ...
  class CaptureCommand
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      raise "Expected non-negative value for Reservation#held" unless after_state >= 0
      [sut, args] && nil
    end
  end
  ...
  class ReleaseCommand
    ...
    def applicable?(state)
      override = HoldCaptureReleasePbtSupport.applicable_override(name)
      return HoldCaptureReleasePbtSupport.call_applicable_override(override, state, nil) if override
      true
    end
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      # Analyzer hints: state_field="available", size_delta=1, transition_kind=nil, requires_non_empty_state=false, scalar_update_kind=:increment_like, command_confidence=:medium, guard_kind=:none, rhs_source_kind=:arg, state_update_shape=:increment
      # Related Alloy property predicates: Hold, Capture, NonNegativeAvailable
      # Related pattern hints: size
      # Derived verify hints: check_size_semantics, check_non_negative_scalar_state
      # Suggested verify order:
      # 1. Command-specific postconditions
