...
JobStatusLifecyclePbtConfig = {
  sut_factory: -> { JobStatusLifecycleImpl.new },
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { sut.status }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
