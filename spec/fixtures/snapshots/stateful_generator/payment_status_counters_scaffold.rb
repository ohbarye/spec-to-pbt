...
RSpec.describe "payment_status_counters (stateful scaffold)" do
  ...
  # - edit payment_status_counters_pbt_config.rb for SUT wiring and durable API mapping
  # - edit payment_status_counters_impl.rb for implementation behavior
  ...
  it "wires a stateful PBT scaffold (customize before enabling)" do
    ...
    Pbt.assert(worker: :none, num_runs: 5, seed: 1) do
      Pbt.stateful(
        ...
        sut: -> { PaymentStatusCountersPbtSupport.sut_factory(-> { PaymentStatusCountersImpl.new }).call },
        max_steps: 20
      )
    end
  end

  class PaymentStatusCountersModel
    def initialize
      @commands = [
        AuthorizeOneCommand.new,
        CaptureOneCommand.new,
        ResetCommand.new
      ]
    end

    def initial_state
      default_state = { status: 0, authorized: 0, captured: 0 } # TODO: replace with a domain-specific structured model state
      PaymentStatusCountersPbtSupport.initial_state(default_state)
    end
  ...
  class AuthorizeOneCommand
    ...
    def applicable?(state)
      override = PaymentStatusCountersPbtSupport.applicable_override(name)
      return PaymentStatusCountersPbtSupport.call_applicable_override(override, state, nil) if override
      return true if PaymentStatusCountersPbtSupport.guard_failure_policy(name)
      guard_satisfied?(state)
    end

    def guard_satisfied?(state, _args = nil)
      state[:status] == 0 # inferred scalar lifecycle/status precondition for authorize_one
    end

    def next_state(state, args)
      override = PaymentStatusCountersPbtSupport.next_state_override(name)
      return PaymentStatusCountersPbtSupport.call_next_state_override(override, state, args) if override
      return state if PaymentStatusCountersPbtSupport.guard_failure_policy(name) && !guard_satisfied?(state, args)
      state.merge(status: 1, authorized: state[:authorized] + 1, captured: state[:captured])
    end

    def run!(sut, args)
      PaymentStatusCountersPbtSupport.before_run_hook&.call(sut)
      payload = PaymentStatusCountersPbtSupport.adapt_args(name, args)
      method_name = PaymentStatusCountersPbtSupport.resolve_method_name(name, :authorize_one)
      result = begin
        if payload.nil?
        ...
        raise unless [:raise, :custom].include?(PaymentStatusCountersPbtSupport.guard_failure_policy(name))
        error
      end
      adapted_result = PaymentStatusCountersPbtSupport.adapt_result(name, result)
      PaymentStatusCountersPbtSupport.after_run_hook&.call(sut, adapted_result)
      adapted_result
    end

    def verify!(before_state:, after_state:, args:, result:, sut:)
      # TODO: translate predicate semantics into postcondition checks
      # Alloy predicate body (preview): "#p.status=0 implies#p'.status=1 and#p'.authorized=add[#p.authorized,1]and#p'.captured=#p.captured"
      ...
      # Related Alloy property predicates: CaptureOne, Reset
      ...
      policy = PaymentStatusCountersPbtSupport.guard_failure_policy(name)
      guard_failed = policy && !guard_satisfied?(before_state, args)
      # Suggested verify order:
      # 1. Command-specific postconditions
      # 2. Related Alloy assertions/facts
      # 3. Related property predicates
      return nil if PaymentStatusCountersPbtSupport.call_verify_override(
        name,
      ...
      raise "Expected incremented value for Payment#authorized" unless after_authorized == before_authorized + 1
      before_captured = before_state[:captured]
      after_captured = after_state[:captured]
      raise "Expected preserved value for Payment#captured" unless after_captured == before_captured
      [sut, args] && nil
    end
  end

  class CaptureOneCommand
    # Analyzer command confidence: medium
    # TODO: confirm this predicate should be modeled as a command
    def name
      :capture_one
    ...
    def guard_satisfied?(state, _args = nil)
      state[:authorized] > 0 # inferred scalar precondition for capture_one
    end
    ...
    def next_state(state, args)
      ...
      return PaymentStatusCountersPbtSupport.call_next_state_override(override, state, args) if override
      return state if PaymentStatusCountersPbtSupport.guard_failure_policy(name) && !guard_satisfied?(state, args)
      state.merge(status: 2, authorized: state[:authorized] - 1, captured: state[:captured] + 1)
    end

    def run!(sut, args)
      PaymentStatusCountersPbtSupport.before_run_hook&.call(sut)
      payload = PaymentStatusCountersPbtSupport.adapt_args(name, args)
      method_name = PaymentStatusCountersPbtSupport.resolve_method_name(name, :capture_one)
      result = begin
        if payload.nil?
          sut.public_send(method_name)
        elsif payload.is_a?(Array)
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      # TODO: translate predicate semantics into postcondition checks
      # Alloy predicate body (preview): "#p.status=1 and#p.authorized>0 implies#p'.status=2 and#p'.authorized=sub[#p.authorized,1]and#p'.captured=add[#p.captured,1]"
      # Analyzer hints: state_field="authorized", size_delta=1, transition_kind=nil, requires_non_empty_state=true, scalar_update_kind=:decrement_like, command_confidence=:medium, guard_kind=:non_empty, guard_field="authorized", guard_constant=nil, rhs_source_kind=:unknown, state_update_shape=:decrement
      # Related Alloy property predicates: AuthorizeOne, Reset
      # Related pattern hints: size
      # Derived verify hints: respect_non_empty_guard, check_size_semantics, check_guard_failure_semantics
      policy = PaymentStatusCountersPbtSupport.guard_failure_policy(name)
      guard_failed = policy && !guard_satisfied?(before_state, args)
      ...
      before_authorized = before_state[:authorized]
      after_authorized = after_state[:authorized]
      raise "Expected decremented value for Payment#authorized" unless after_authorized == before_authorized - 1
      before_captured = before_state[:captured]
      after_captured = after_state[:captured]
      raise "Expected incremented value for Payment#captured" unless after_captured == before_captured + 1
      [sut, args] && nil
    end
  end
  ...
  class ResetCommand
    ...
    def next_state(state, args)
      ...
      return PaymentStatusCountersPbtSupport.call_next_state_override(override, state, args) if override
      return state if PaymentStatusCountersPbtSupport.guard_failure_policy(name) && !guard_satisfied?(state, args)
      state.merge(status: 0, authorized: 0, captured: 0)
    end

    def run!(sut, args)
      PaymentStatusCountersPbtSupport.before_run_hook&.call(sut)
      payload = PaymentStatusCountersPbtSupport.adapt_args(name, args)
      method_name = PaymentStatusCountersPbtSupport.resolve_method_name(name, :reset)
      result = begin
        if payload.nil?
          sut.public_send(method_name)
        elsif payload.is_a?(Array)
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      # TODO: translate predicate semantics into postcondition checks
      # Alloy predicate body (preview): "#p.status=2 implies#p'.status=0 and#p'.authorized=0 and#p'.captured=0"
      ...
      # Related Alloy property predicates: AuthorizeOne, CaptureOne
      # Related pattern hints: size
      # Derived verify hints: respect_non_empty_guard, check_size_semantics, check_guard_failure_semantics
      policy = PaymentStatusCountersPbtSupport.guard_failure_policy(name)
      guard_failed = policy && !guard_satisfied?(before_state, args)
      ...
      raise "Expected replaced value for Payment#authorized" unless after_authorized == expected_authorized
      before_captured = before_state[:captured]
      after_captured = after_state[:captured]
      expected_captured = 0
      raise "Expected replaced value for Payment#captured" unless after_captured == expected_captured
      [sut, args] && nil
    end
  end
