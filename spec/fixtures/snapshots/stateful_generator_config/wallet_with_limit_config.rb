...
WalletWithLimitPbtConfig = {
  ...
  # initial_state: { balance: 0, credit_limit: 3 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    deposit: {
      method: :deposit,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { balance: sut.balance, credit_limit: sut.credit_limit } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
