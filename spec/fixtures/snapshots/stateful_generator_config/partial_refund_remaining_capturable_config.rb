...
PartialRefundRemainingCapturablePbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { authorized: sut.authorized, captured: sut.captured, refunded: sut.refunded } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
