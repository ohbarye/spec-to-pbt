...
RateLimiterPbtConfig = {
  ...
  # initial_state: { remaining: 0, capacity: 3 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    allow: {
      method: :allow,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { remaining: sut.remaining, capacity: sut.capacity } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
