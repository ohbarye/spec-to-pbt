...
JobQueueRetryDeadLetterPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { ready: sut.ready, in_flight: sut.in_flight, dead_letter: sut.dead_letter } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
