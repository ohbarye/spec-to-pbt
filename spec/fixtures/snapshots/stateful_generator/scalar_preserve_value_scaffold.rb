...
require "pbt"
require_relative "box_impl" if File.exist?(File.expand_path("box_impl.rb", __dir__))
...
RSpec.describe "box (stateful scaffold)" do
  ...
  module BoxPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in BoxPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in BoxPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: BoxPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: BoxPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  class KeepCommand
    ...
    def arguments
      overridden = BoxPbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(BoxPbtSupport::ARGUMENTS_OVERRIDE_UNSET)
      Pbt.nil
    end
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      # Analyzer hints: state_field="value", size_delta=0, transition_kind=nil, requires_non_empty_state=false, scalar_update_kind=:replace_like, command_confidence=:medium, guard_kind=:none, guard_field="value", guard_constant=nil, rhs_source_kind=:state_field, state_update_shape=:preserve_value
      ...
      observed = BoxPbtSupport.observed_state(sut)
      if !observed.nil?
