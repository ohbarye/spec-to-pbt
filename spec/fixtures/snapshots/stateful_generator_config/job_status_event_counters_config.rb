...
JobStatusEventCountersPbtConfig = {
  ...
  # initial_state: { status: 0, events: [], retry_budget: 0, retry_count: 0 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    activate: {
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { events: sut.events.dup, status: sut.status, retry_budget: sut.retry_budget, retry_count: sut.retry_count } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
