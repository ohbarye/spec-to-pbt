...
PartialRefundRemainingCapturablePbtConfig = {
  ...
  # initial_state: { authorized: 0, captured: 0, refunded: 0 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    capture: {
      ...
      # arguments_override: ->(state) { Pbt.integer(min: 1, max: state[:authorized] + 1) }, # optional 0-arity/1-arity generator override for invalid-path coverage or custom distributions
      ...
      # Invalid-path starting point: set verify_context.state_reader first, then try arguments_override: ->(state) { Pbt.integer(min: 1, max: state[:authorized] + 1) } with guard_failure_policy: :raise.
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { authorized: sut.authorized, captured: sut.captured, refunded: sut.refunded } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
