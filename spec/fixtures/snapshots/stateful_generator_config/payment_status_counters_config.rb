...
PaymentStatusCountersPbtConfig = {
  ...
  # initial_state: { status: 0, authorized: 0, captured: 0 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    authorize_one: {
      method: :authorize_one, # TODO(required): replace with real method name
    ...
    capture_one: {
      method: :capture_one, # TODO(required): replace with real method name (suggested: :capture, :settle)
      # arg_adapter: ->(args) { args }, # use when the Ruby API wants a different runtime argument shape
      # model_arg_adapter: ->(args) { args }
      # result_adapter: ->(result) { result }, # use when the SUT result needs normalization before verify!
      # arguments_override: -> { Pbt.nil }, # optional 0-arity/1-arity generator override for invalid-path coverage or custom distributions
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { status: sut.status, authorized: sut.authorized, captured: sut.captured } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
