...
require "pbt"
require_relative "payment_status_amounts_impl" if File.exist?(File.expand_path("payment_status_amounts_impl.rb", __dir__))
...
RSpec.describe "payment_status_amounts (stateful scaffold)" do
  ...
  module PaymentStatusAmountsPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in PaymentStatusAmountsPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in PaymentStatusAmountsPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: PaymentStatusAmountsPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: PaymentStatusAmountsPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    PaymentStatusAmountsPbtSupport.validate_config!
