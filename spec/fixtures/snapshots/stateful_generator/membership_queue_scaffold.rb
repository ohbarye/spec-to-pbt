...
require "pbt"
require_relative "membership_queue_impl" if File.exist?(File.expand_path("membership_queue_impl.rb", __dir__))
...
RSpec.describe "membership_queue (stateful scaffold)" do
  ...
  module MembershipQueuePbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in MembershipQueuePbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in MembershipQueuePbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: MembershipQueuePbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: MembershipQueuePbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    MembershipQueuePbtSupport.validate_config!
  ...
  class EnqueueCommand
    ...
    def arguments
      overridden = MembershipQueuePbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(MembershipQueuePbtSupport::ARGUMENTS_OVERRIDE_UNSET)
