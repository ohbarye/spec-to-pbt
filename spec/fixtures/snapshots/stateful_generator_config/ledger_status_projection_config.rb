...
LedgerStatusProjectionPbtConfig = {
  ...
  # initial_state: { status: 0, entries: [], balance: 0 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    open: {
    ...
    post_amount: {
      method: :post_amount, # TODO(required): replace with real method name
      # arg_adapter: ->(args) { args }, # use when the Ruby API wants a different runtime argument shape
      # model_arg_adapter: ->(args) { args }
      # result_adapter: ->(result) { result }, # use when the SUT result needs normalization before verify!
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { entries: sut.entries.dup, status: sut.status, balance: sut.balance } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
