...
require "pbt"
require_relative "queue_impl" if File.exist?(File.expand_path("queue_impl.rb", __dir__))
...
RSpec.describe "queue (stateful scaffold)" do
  ...
  module QueuePbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in QueuePbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in QueuePbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: QueuePbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: QueuePbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    QueuePbtSupport.validate_config!
  ...
  class EnqueueCommand
    ...
    def arguments
      overridden = QueuePbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(QueuePbtSupport::ARGUMENTS_OVERRIDE_UNSET)
