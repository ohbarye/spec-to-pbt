...
# Regeneration-safe customization file for thermostat stateful scaffold.
...
ThermostatPbtConfig = {
  ...
  command_mappings: {
    set_target: {
      method: :set_target, # TODO(required): replace with real method name
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { sut.target } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
