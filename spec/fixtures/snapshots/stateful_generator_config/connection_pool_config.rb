...
ConnectionPoolPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { available: sut.available, checked_out: sut.checked_out, capacity: sut.capacity } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
