...
FeatureFlagRolloutPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { rollout: sut.rollout, max_rollout: sut.max_rollout } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
