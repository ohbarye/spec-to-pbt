...
JobStatusEventCountersPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { events: sut.events.dup, status: sut.status, retry_budget: sut.retry_budget, retry_count: sut.retry_count } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
