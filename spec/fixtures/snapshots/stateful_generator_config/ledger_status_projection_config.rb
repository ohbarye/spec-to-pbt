...
LedgerStatusProjectionPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { entries: sut.entries.dup, status: sut.status, balance: sut.balance } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
