...
PaymentStatusCountersPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { status: sut.status, authorized: sut.authorized, captured: sut.captured } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
