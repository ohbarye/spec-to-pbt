...
WalletWithLimitPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { balance: sut.balance, credit_limit: sut.credit_limit } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
