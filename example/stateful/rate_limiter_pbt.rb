...
require "pbt"
require_relative "rate_limiter_impl" if File.exist?(File.expand_path("rate_limiter_impl.rb", __dir__))
...
RSpec.describe "rate_limiter (stateful scaffold)" do
  ...
  module RateLimiterPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in RateLimiterPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in RateLimiterPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: RateLimiterPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: RateLimiterPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    RateLimiterPbtSupport.validate_config!
  ...
  class AllowCommand
    ...
    def arguments
      overridden = RateLimiterPbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(RateLimiterPbtSupport::ARGUMENTS_OVERRIDE_UNSET)
      Pbt.nil
    end
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      # Analyzer hints: state_field="remaining", size_delta=-1, transition_kind=nil, requires_non_empty_state=true, scalar_update_kind=:decrement_like, command_confidence=:medium, guard_kind=:non_empty, guard_field="remaining", guard_constant=nil, rhs_source_kind=:unknown, state_update_shape=:decrement
  ...
  class ResetCommand
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      # Analyzer hints: state_field="remaining", size_delta=0, transition_kind=nil, requires_non_empty_state=false, scalar_update_kind=:replace_like, command_confidence=:medium, guard_kind=:none, guard_field="remaining", guard_constant=nil, rhs_source_kind=:state_field, state_update_shape=:replace_value
