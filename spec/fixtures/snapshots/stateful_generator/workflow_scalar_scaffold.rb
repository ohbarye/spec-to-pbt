...
require "pbt"
require_relative "workflow_impl" if File.exist?(File.expand_path("workflow_impl.rb", __dir__))
...
RSpec.describe "workflow (stateful scaffold)" do
  ...
  module WorkflowPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in WorkflowPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in WorkflowPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: WorkflowPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: WorkflowPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    WorkflowPbtSupport.validate_config!
  ...
  class WorkflowModel
    ...
    def initial_state
      ...
      WorkflowPbtSupport.initial_state(default_state)
    end
  ...
  class StepCommand
    ...
    def arguments
      overridden = WorkflowPbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(WorkflowPbtSupport::ARGUMENTS_OVERRIDE_UNSET)
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      observed = WorkflowPbtSupport.observed_state(sut)
      if !observed.nil?
