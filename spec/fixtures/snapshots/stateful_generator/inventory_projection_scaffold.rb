...
require "pbt"
require_relative "inventory_projection_impl" if File.exist?(File.expand_path("inventory_projection_impl.rb", __dir__))
...
RSpec.describe "inventory_projection (stateful scaffold)" do
  ...
  module InventoryProjectionPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in InventoryProjectionPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in InventoryProjectionPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: InventoryProjectionPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: InventoryProjectionPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    InventoryProjectionPbtSupport.validate_config!
