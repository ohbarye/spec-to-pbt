...
JobStatusCountersPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { status: sut.status, retry_count: sut.retry_count, dead_letter_count: sut.dead_letter_count } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
