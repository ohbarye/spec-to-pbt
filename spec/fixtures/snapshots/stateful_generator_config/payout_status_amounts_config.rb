...
PayoutStatusAmountsPbtConfig = {
  ...
  # initial_state: { status: 0, pending_amount: 0, paid_amount: 0 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    queue_amount: {
      method: :queue_amount,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { status: sut.status, pending_amount: sut.pending_amount, paid_amount: sut.paid_amount } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
