...
BoundedQueuePbtConfig = {
  ...
  # initial_state: { elements: [], capacity: 3 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    enqueue: {
      method: :enqueue,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { sut.snapshot }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
