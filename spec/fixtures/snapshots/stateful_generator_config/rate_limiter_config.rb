...
RateLimiterPbtConfig = {
  ...
  # initial_state: { remaining: 0, capacity: 3 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    allow: {
      method: :allow, # TODO(required): replace with real method name
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { remaining: sut.remaining, capacity: sut.capacity } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
