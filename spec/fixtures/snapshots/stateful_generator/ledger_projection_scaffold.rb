...
RSpec.describe "ledger_projection (stateful scaffold)" do
  ...
  module LedgerProjectionPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in LedgerProjectionPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in LedgerProjectionPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: LedgerProjectionPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: LedgerProjectionPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold (customize before enabling)" do
    ...
    LedgerProjectionPbtSupport.validate_config!
  ...
  class PostCreditCommand
    ...
    def arguments
      overridden = LedgerProjectionPbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(LedgerProjectionPbtSupport::ARGUMENTS_OVERRIDE_UNSET)
      Pbt.integer
    end
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      # Analyzer hints: state_field="entries", size_delta=1, transition_kind=:append, requires_non_empty_state=false, scalar_update_kind=nil, command_confidence=:high, guard_kind=:none, guard_field="entries", guard_constant=nil, rhs_source_kind=:unknown, state_update_shape=:append_like
      ...
      observed = LedgerProjectionPbtSupport.observed_state(sut)
      if !observed.nil?
