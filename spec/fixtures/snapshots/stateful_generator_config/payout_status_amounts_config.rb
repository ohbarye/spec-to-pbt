...
PayoutStatusAmountsPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { status: sut.status, pending_amount: sut.pending_amount, paid_amount: sut.paid_amount } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
