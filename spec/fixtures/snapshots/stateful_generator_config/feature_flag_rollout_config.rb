...
FeatureFlagRolloutPbtConfig = {
  ...
  # initial_state: { rollout: 0, max_rollout: 3 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    enable: {
      method: :enable,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { rollout: sut.rollout, max_rollout: sut.max_rollout } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
