...
InventoryStatusProjectionPbtConfig = {
  ...
  # initial_state: { status: 0, movements: [], stock: 0 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    activate: {
      method: :activate,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { movements: sut.movements.dup, status: sut.status, stock: sut.stock } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
