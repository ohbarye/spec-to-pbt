...
JobStatusCountersPbtConfig = {
  ...
  # initial_state: { status: 0, retry_count: 0, dead_letter_count: 0 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    start: {
      method: :start,
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { status: sut.status, retry_count: sut.retry_count, dead_letter_count: sut.dead_letter_count } }; configure this when verify_override needs observed-state checks against the SUT
  }
  # before_run: ->(sut) { },
