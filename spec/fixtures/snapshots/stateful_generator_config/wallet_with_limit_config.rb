...
WalletWithLimitPbtConfig = {
  ...
  # initial_state: { balance: 0, credit_limit: 3 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    deposit: {
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { balance: sut.balance, credit_limit: sut.credit_limit } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
