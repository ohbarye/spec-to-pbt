...
TransferBetweenAccountsPbtConfig = {
  ...
  # initial_state: { source_balance: 0, target_balance: 0 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    transfer: {
      method: :transfer, # TODO(required): replace with real method name (suggested: :move_funds, :transfer_amount, :post_transfer)
      ...
      # arguments_override: ->(state) { Pbt.integer(min: 1, max: state[:source_balance] + 1) }, # optional 0-arity/1-arity generator override for invalid-path coverage or custom distributions
      ...
      # Invalid-path starting point: set verify_context.state_reader first, then try arguments_override: ->(state) { Pbt.integer(min: 1, max: state[:source_balance] + 1) } with guard_failure_policy: :raise.
      ...
      # verify_override: ->(after_state:, observed_state:, **) { raise \"Expected observed account balances to match model\" unless observed_state == after_state } # leave this commented out when state_reader already exposes the model-shaped observed state; enable it for custom postconditions or invalid-path semantics
    }
  },
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { { source_balance: sut.source_balance, target_balance: sut.target_balance } } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
