...
# Regeneration-safe customization file for job_status_lifecycle stateful scaffold.
...
JobStatusLifecyclePbtConfig = {
  ...
  command_mappings: {
    start: {
      method: :start, # TODO(required): replace with real method name
    ...
    complete: {
      method: :complete, # TODO(required): replace with real method name
      # arg_adapter: ->(args) { args }, # use when the Ruby API wants a different runtime argument shape
      # model_arg_adapter: ->(args) { args }
      # result_adapter: ->(result) { result }, # use when the SUT result needs normalization before verify!
      # arguments_override: -> { Pbt.nil }, # optional 0-arity/1-arity generator override for invalid-path coverage or custom distributions
    ...
    fail: {
      method: :fail, # TODO(required): replace with real method name
      # arg_adapter: ->(args) { args }, # use when the Ruby API wants a different runtime argument shape
      # model_arg_adapter: ->(args) { args }
      # result_adapter: ->(result) { result }, # use when the SUT result needs normalization before verify!
      # arguments_override: -> { Pbt.nil }, # optional 0-arity/1-arity generator override for invalid-path coverage or custom distributions
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { sut.status } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
