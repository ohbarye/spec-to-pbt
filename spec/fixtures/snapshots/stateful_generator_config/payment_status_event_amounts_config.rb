...
PaymentStatusEventAmountsPbtConfig = {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { captures: sut.captures.dup, status: sut.status, remaining_amount: sut.remaining_amount, settled_amount: sut.settled_amount } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
