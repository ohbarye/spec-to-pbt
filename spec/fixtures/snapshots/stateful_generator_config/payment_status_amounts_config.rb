...
PaymentStatusAmountsPbtConfig = {
  ...
  # initial_state: { status: 0, authorized_amount: 0, captured_amount: 0 }, # set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    authorize_amount: {
  ...
  verify_context: {
    state_reader: nil, # suggested: ->(sut) { { status: sut.status, authorized_amount: sut.authorized_amount, captured_amount: sut.captured_amount } }; configure this when observed-state checks should be compared against the model
  }
  # before_run: ->(sut) { },
