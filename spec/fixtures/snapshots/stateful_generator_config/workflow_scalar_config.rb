...
# Regeneration-safe customization file for workflow stateful scaffold.
...
WorkflowPbtConfig = {
  ...
  command_mappings: {
    step: {
      method: :step, # TODO(required): replace with real method name
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { sut.value } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
