...
require "pbt"
require_relative "thermostat_impl" if File.exist?(File.expand_path("thermostat_impl.rb", __dir__))
...
RSpec.describe "thermostat (stateful scaffold)" do
  ...
  module ThermostatPbtSupport
    ...
    def validate_config!
      ...
      raise "Unknown config keys in ThermostatPbtConfig: #{unknown_top.inspect}. See docs/config-reference.md for valid keys." unless unknown_top.empty?
      ...
      config.fetch(:command_mappings, {}).each do |cmd_name, cmd_config|
        ...
        raise "Unknown config keys in ThermostatPbtConfig command_mappings[#{cmd_name}]: #{unknown_cmd.inspect}. See docs/config-reference.md for valid keys." unless unknown_cmd.empty?
      end

      if !config.key?(:sut_factory)
        warn "Warning: ThermostatPbtConfig is missing :sut_factory (required). The scaffold will use a default factory."
      end

      if state_reader.nil?
        warn "Warning: ThermostatPbtConfig has no verify_context.state_reader configured. SUT state will not be compared against the model."
      end
    end
  ...
  it "wires a stateful PBT scaffold" do
    ThermostatPbtSupport.validate_config!
  ...
  class ThermostatModel
    ...
    def initial_state
      ...
      ThermostatPbtSupport.initial_state(default_state)
    end
  ...
  class SetTargetCommand
    ...
    def arguments
      overridden = ThermostatPbtSupport.call_arguments_override(name)
      return overridden unless overridden.equal?(ThermostatPbtSupport::ARGUMENTS_OVERRIDE_UNSET)
      Pbt.integer
    end
    ...
    def next_state(state, args)
      override = ThermostatPbtSupport.next_state_override(name)
      return ThermostatPbtSupport.call_next_state_override(override, state, args) if override
    ...
    def verify!(before_state:, after_state:, args:, result:, sut:)
      ...
      observed = ThermostatPbtSupport.observed_state(sut)
      if !observed.nil?
