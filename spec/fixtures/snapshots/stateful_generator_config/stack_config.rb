...
StackPbtConfig = {
  ...
  command_mappings: {
    push_adds_element: {
      method: :push_adds_element, # TODO(required): replace with real method name (suggested: :push)
    ...
    pop_removes_element: {
      method: :pop_removes_element, # TODO(required): replace with real method name (suggested: :pop)
      # arg_adapter: ->(args) { args }, # use when the Ruby API wants a different runtime argument shape
      # model_arg_adapter: ->(args) { args }
      # result_adapter: ->(result) { result }, # use when the SUT result needs normalization before verify!
