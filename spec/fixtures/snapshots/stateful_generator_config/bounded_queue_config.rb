...
BoundedQueuePbtConfig = {
  ...
  # initial_state: { elements: [], capacity: 3 }, # optional: set this when the inferred model baseline does not match your SUT defaults
  command_mappings: {
    enqueue: {
      method: :enqueue, # TODO(required): replace with real method name
      ...
      # verify_override: ->(after_state:, observed_state:, **) { raise \"Expected observed collection state to match model\" unless observed_state == after_state[:elements] } # leave this commented out when state_reader already exposes the model-shaped observed state; enable it for custom postconditions or invalid-path semantics
    },
    dequeue: {
      method: :dequeue, # TODO(required): replace with real method name
      # arg_adapter: ->(args) { args }, # use when the Ruby API wants a different runtime argument shape
      # model_arg_adapter: ->(args) { args }
      # result_adapter: ->(result) { result }, # use when the SUT result needs normalization before verify!
  ...
  verify_context: {
    state_reader: nil, # TODO(required): suggested: ->(sut) { sut.snapshot } — without this, SUT state is not compared against the model
  }
  # before_run: ->(sut) { },
